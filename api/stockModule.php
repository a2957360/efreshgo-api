    ...
    if(isset($data['isGet']) && $data['isGet'] !== ""){
      ...
      switch ($orderBy) {
        case 'normal':
          ...
          break;
        case 'new':
          $orderBy = " ORDER BY `itemTable`.`createTime` DESC";
          break;
        case 'sale':
          $orderBy = " ORDER BY `itemTable`.`itemSaleNum` DESC";
          break;
        default:
          # code...
          break;
      ...
      //产品搜索
      $stmt = $pdo->prepare("SELECT `itemTable`.*,`savedItemTable`.`savedItemId` AS `savedItemNumber`
                      From `itemTable` 
                      LEFT JOIN `savedItemTable` ON `itemTable`.`itemNumber` = `savedItemTable`.`itemNumber` AND `savedItemTable`.`userNumber` = '$userNumber'
                      WHERE `itemTable`.`language` = '$language' ".$searchSql." GROUP BY `itemTable`.`itemNumber` ".$orderBy." limit $offset,$limit");
      $stmt->execute();
      if($stmt != null){
        while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
          //所以查询都是要用number
      ...
      if(isset($itemNumber) && $itemNumber != "" && $itemNumber != null){
        ...
        // $searchSql .= isset($storeNumber)?" AND `stockTable`.`storeNumber`='$storeNumber'":"";
        $likesearchSql .= isset($itemCategory)?" AND FIND_IN_SET('".$itemCategory."',`itemTable`.`itemCategory`) AND `itemTable`.`itemNumber` !='$itemNumber'":"";

        $likeList = array();
        $stmt = $pdo->prepare("SELECT `itemTable`.*,`savedItemTable`.`savedItemId` AS `savedItemNumber`
                              From `itemTable` 
                              LEFT JOIN `savedItemTable` ON `itemTable`.`itemNumber` = `savedItemTable`.`itemNumber` AND `savedItemTable`.`userNumber` = '$userNumber'
                              WHERE `itemTable`.`language` = '$language' ".$likesearchSql." ORDER BY RAND() limit $limit");
        $stmt->execute();
        if($stmt != null){
          while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
            //所以查询都是要用number
